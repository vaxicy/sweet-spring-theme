"""Compose every Sweet Spring store asset from one HTML/CSS source.

Layering and geometry mirror a real installed-Chrome screenshot (1080x646
capture of a maximized window). The window is authored in that capture's pixel
space and scaled once per output:
...
C['deep'] = [74, 46, 59]     # dark plum for text on the pink tile/cards
C['blue'] = C['background_tab']  # alias used by the avatar
...
# ---------------------------------------------------------------------------
LOGO = '#EE6FA8'          # Chrome computes the NTP Google mark (ntp_logo_alternate) -> pink
CIRCLE = '#F9B2D7'        # Chrome tints the shortcut tiles from ntp_background -> pink
OMNI_BORDER = '#D3DAE3'   # unfocused omnibox outline on the green toolbar
...
PILL_FG = '#F8BBD4'       # its label + pencil (pink tint on dark pill)
WIN_BTN = '#2B2B2B'       # minimize / maximize / close glyphs
FOLDER = '#F4B400'        # bookmark folder glyphs (yellow, as rendered by Chrome)
G_RED, G_BLUE, G_YELLOW, G_GREEN = '#EA4335', '#4285F4', '#FBBC05', '#34A853'

VARS = f""":root{{
  --pink:{color('frame')};
  --green:{color('toolbar')};
  --blue:{color('background_tab')};
...
.tabstrip{height:32px;background:var(--pink);display:flex;align-items:flex-end;padding-left:33px}
...
.tab{width:168px;height:27px;border-radius:9px 9px 0 0;margin-right:7px;padding:0 10px 0 30px;display:flex;align-items:center;
     font-size:11.5px;color:var(--ink);position:relative;outline:1px solid rgba(255,255,255,.35);outline-offset:-1px}
.tab.on{background:var(--milk);outline:0;color:var(--ink)}
.tab i{position:absolute;left:11px;top:7px;width:12px;height:12px;border-radius:3px;background:rgba(90,50,70,.22)}
...
/* ---- toolbar ---- */
.toolbar{height:32px;background:var(--green);display:flex;align-items:center;gap:16px;padding:0 14px}
.nav{display:flex;gap:15px;align-items:center}
.omni{flex:1;height:27px;border:1.5px solid var(--omni-border);border-radius:14px;background:#FFFFFF;display:flex;align-items:center;
...
/* ---- bookmark bar ---- */
.bookmarks{height:32px;background:var(--green);display:flex;align-items:center;gap:19px;padding:0 14px;font-size:11.5px;color:var(--ink)}
.bookmarks .sep{width:1px;height:14px;background:rgba(90,50,70,.25)}
...
.shortcut{width:88px;text-align:center;font-size:12px;color:var(--ui-text)}
...
.customize{position:absolute;right:10px;bottom:10px;height:26px;border-radius:13px;background:#202124;color:#F8BBD4;
           display:flex;align-items:center;gap:6px;padding:0 12px;font-size:11.5px}

/* ---- promo: 440x280 brand tile ---- */
.tile{width:440px;height:280px;background:var(--pink);position:relative;overflow:hidden;text-align:center;color:var(--deep)}
.tile img{width:74px;height:74px;display:block;margin:26px auto 0}
...
.tile:after{content:'';position:absolute;left:0;right:0;bottom:0;height:14px;background:var(--green)}
...
.marquee .frame{position:absolute;top:139px;left:300px;width:800px;height:370px;overflow:hidden;border:2px solid var(--pink);
                border-radius:16px;box-shadow:0 10px 30px rgba(90,50,70,.13)}
...
def avatar(size):
    return (f'<svg width="{size}" height="{size}" viewBox="0 0 24 24">'
            f'<circle cx="12" cy="12" r="12" fill="{color("blue")}"/>'
            f'<circle cx="12" cy="9.4" r="4.1" fill="#FFFFFF"/>'
            f'<path d="M3.8 21.2c1.6-4.3 4.7-6.4 8.2-6.4s6.6 2.1 8.2 6.4z" fill="#FFFFFF"/></svg>')
...
def bookmarks():
    kids = ''.join(
        f'<div class="bm"><svg width="13" height="13" viewBox="0 0 24 24">'
        f'<path d="M3 7.5h6l2 2.5h10v8.5a1.5 1.5 0 0 1-1.5 1.5h-15A1.5 1.5 0 0 1 3 18.5z" fill="{FOLDER}"/></svg>{name}</div>'
        for name in ('Tools', 'Design', 'Dev', 'Reading'))
    return ('<div class="bookmarks">' + apps(13, color('ntp_text')) + '<div class="sep"></div>' + kids + '</div>')


def window(height=675, tabs=('New Tab', 'Reading list', 'Design ideas', 'Sweet Spring')):
...
tile = ('<div class="tile">' + f'<img src="{LOGO_URI}" alt="Sweet Spring logo">'
        + '<h1>Sweet Spring</h1><div class="kicker">CHROME THEME</div>'
        + '<p>A pastel garden for every new tab.</p></div>')
marquee = ('<div class="marquee"><h1>Sweet Spring Theme</h1><p>Cherry pink, spring green and calm blue.</p>'
           + '<div class="frame">' + window(height=500) + '</div></div>')

PALETTE = [
    ('Soft Pink', 'frame', 'Window frame', 'deep'),
    ('Spring Green', 'toolbar', 'Toolbar & active tab', 'ntp_text'),
    ('Calm Blue', 'background_tab', 'Background tabs', 'ntp_text'),
    ('White Pink', 'ntp_background', 'New tab background', 'ntp_text'),
]
cards = ''.join(
    f'<div class="card" style="background:{color(k)};color:{color(fg)};border:1px solid rgba(90,50,70,.16)">'
    f'<strong>{name}</strong><span>{color(k)} · {role}</span></div>' for name, k, role, fg in PALETTE)
intro = ('<div class="intro"><div class="kicker">A PASTEL SPRING GARDEN</div><h1>Sweet Spring Theme</h1>'
         '<p class="lead">Four colors. One soft, blooming space.</p><div class="cards">' + cards + '</div>'
         '<p class="chips">Solid colors · Gentle pastels · Flat design</p></div>')
